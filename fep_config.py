...
def config_gen_eq(pdb_name,ligand,type):
	a = str(os.path.splitext(pdb_name)[0])
	file_name_wh_ex = a[0:]
	structure = parsePDB(str(pdb_name))
	
	before,sep,after = a.rpartition('.')
	bf,sp,ar = before.rpartition('_')
	tb,tp,tr = bf.rpartition('_')
	tr=tr[:-2]
	...
	f.write("parameters              par_all27_prot_lipid_na.inp\n")
	f.write("parameters              %s.par\n"% str(ligand))
...
""")
	f.write("\n")
	if type in ('ann','cre'):
		f.write("# COLVARS SECTION\n")
		f.write("colvars 				on\n")
		f.write("colvarsConfig 			dist_%s_%s.in\n" % (str(type),str(tr))) 
	f.write("""
...
def config_gen_fw(pdb_name,ligand,type,window_size):
	a = str(os.path.splitext(pdb_name)[0])
	file_name_wh_ex = a[0:]
	structure = parsePDB(str(pdb_name))
...
def config_gen_bw(pdb_name,ligand,type,window_size):
	a = str(os.path.splitext(pdb_name)[0])
	file_name_wh_ex = a[0:]
	structure = parsePDB(str(pdb_name))
...
lig = input("What is the ligand in the system: ")
type = input("ann or cre ?")


config_gen_eq(pdbname,lig,type)
config_gen_fw(pdbname,lig,type,w_size)
config_gen_bw(pdbname,lig,type,w_size)